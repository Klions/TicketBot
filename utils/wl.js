module.exports = {
  async wl(interaction, client, id_usuario, tipo) {
    ...
    var TextoLiberar = "revogou";
    if(tipo == 1){
      TextoLiberar = "liberou";
    }
    ...
    if(VRP_WLUSER){
      ...
      client.embedcustom(interaction.channel.id, {
        mention: usuario.id,
        texto: "<@"+interaction.user.id+"> "+TextoLiberar+" a Whitelist de <@"+usuario.id+"> (`ID: "+VRP_WLUSER.user_id+"`).",
        cor: ticket.category.color
      }, client);
      await client.mysql_query(usuario.id, "UPDATE vrp_users SET whitelisted = "+tipo+" WHERE id = USERID");
      ...
      await interaction.update({ content: 'Este usuário não possui uma conta no servidor para ser liberado ou revogado a Whitelist.', components: [] }).catch(e => console.log(e));
    }
  }
};